# WLFPIR8 Simple PY Media Device
...
# Neopixel ring, set the amount of pixels (only works for one ring)
ring = neopixel.NeoPixel(board.MISO, 20, brightness=0.2, auto_write=False)
...
while True:
    ...
    hue = colorwheel(encoder.position * 4 % 255)  # hue change based on pos

    # LED ring goes off after a time of no activity
    if encoder_diff != 0 or button.value == False:
        last_time = time.monotonic()
    ring_on = ring_timeout == 0 or time.monotonic() - last_time < ring_timeout
